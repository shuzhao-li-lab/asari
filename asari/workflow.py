...
def single_sample_EICs_(sample_id, infile, ion_mode, database_mode,
    ...
    infile : 
        input mzML file, passed from sample_registry by make_iter_parameters.
...
def process_xics(list_input_files, parameters):
    ...
    ----------
    list_input_files : 
        list of centroided mzML files from LC-MS metabolomics. 
...
def get_mz_list(infile):
    ...
    ----------
    infile : 
        input table, which is tab or comma delimited and has m/z in the first column,
        header as first row.
