...
class Sample:
    ...
    def _match_mass_formula_(self, indexed_DB):
        '''
        Match to formula_mass database (tier 1) for high-selectivity (> 0.9) peaks;
        including mass accuracy check, a 2-step RANSAC like fitting. 
        Hard limit is 20 ppm. 
        If mass shift is larger than 20 ppm, user should do own mass calibrate first.
        ...
        hot_DB being the features already seen in this expt, with pointer (peak -> empCpd)

        Maybe empCpd needs full implementation in mass2chem

        After self._match_mass_formula_1_(), we have a list of anchor mass traces.
        Their primary DB match is related to one or more neutral formula.
        This function extends the first (already prioritized in DB) neutral formula to 
        a list of isotopes and adducts, as specified in mass2chem.formula.
        ...
        for M in self.list_MassTraces:
            if M.selectivity > 0.9:
                _matched = search_formula_mass_db(M.mz, indexed_DB)
                if _matched: 
                    M.formula_mass = _matched
                    M.ppm_from_formula_mass = 1000000 * (M.mz - M.formula_mass[1])/M.mz

        self.__mass_accuracy__, self.__mass_stdev__ = normal_distribution.fit(
                                [M.ppm_from_formula_mass for M in self.list_MassTraces if M.formula_mass])

        if abs(self.__mass_accuracy__) > 5: # this is considered significant mass shift, requiring m/z correction for all traces
            for M in self.list_MassTraces:
                M.raw_mz = M.mz
                M.mz = M.mz - self.__mass_accuracy__*0.000001*M.mz
                M.__mass_corrected_by_asari__ = True 
                # redo DB search too, due to mass correction
                if M.selectivity > 0.9:
                    # Also change search ppm if self.__mass_stdev__ > 5
                    _matched = search_formula_mass_db(M.mz, indexed_DB, max(10, 2*self.__mass_stdev__))
                    if _matched:
                        M.formula_mass = _matched
                        M.ppm_from_formula_mass = 1000000 * (M.mz - M.formula_mass[1])/M.mz

            # update __mass_stdev__
            _mu, self.__mass_stdev__ = normal_distribution.fit(
                                [M.ppm_from_formula_mass for M in self.list_MassTraces if M.formula_mass])

        elif self.__mass_stdev__ > 5:      # no mass correction but need redo search using higher ppm for unmatched MassTraces
            for M in self.list_MassTraces:
                if not M.formula_mass and M.selectivity > 0.9:
                    M.formula_mass = search_formula_mass_db(M.mz, indexed_DB, 2*self.__mass_stdev__)

        '''
        mzList = [M.mz for M in self.list_MassTraces]
        formula_mass_annotation = annotate_formula_mass(mzList, indexed_DB, check_mass_accuracy=True, mode=self.mode)
